...
先保留已经验证过的 `hfl/rbt3@0aa0527ff4170f29e1dfd3eb6ef60dc67e1bf75c`，建立一个新的、独立命名的 reasoning-specific corrective lineage。第一轮只做 reasoning head 的三 seed 诊断：RBT3 的 embeddings 和除最后一个 Transformer block 外的 Encoder 参数冻结，只训练最后一个 Transformer block 与 `reasoning_tags` head；六个其他 head 不参与这轮反向传播。纠正变量只有“reasoning-only last-block adaptation”，不同时改变 tokenizer、split、seed、阈值或数据权重。这样可以直接检验 S2 的负迁移是否来自共享七头更新，又不会把现有 S1/S2/S3 artifact 改写成新结果。
...
1. 三个 run units：seed 35、71、107；只用 Train 1,822 拟合，Dev 448 做 primary early stopping/diagnostic；不读 Test、Anchor、Gold、OOD、reference predictions。
2. 继续使用 `hfl/rbt3@0aa0527ff4170f29e1dfd3eb6ef60dc67e1bf75c` 的现有本地 cache；Encoder embeddings 与前两个 Transformer blocks 冻结，只解冻最后一个 block 和 `reasoning_tags` head。六个其他 head 不反向更新。
3. 保持 max length 256、batch 16、最多 12 epochs、patience 3、gradient clipping 1.0、seed 固定、MPS-first/CPU-fallback、每 run 120 分钟和新增磁盘 10 GiB；建议 block LR `1e-5`、reasoning-head LR `5e-4`、AdamW weight decay `0.01`，并在 fit 前冻结所有数值。
4. 每个 seed 保存 block+head checkpoint，做 CPU reload 并检查 Macro-F1、Micro-F1、exact-set accuracy 和全部 15 label 的有限输出；任何非有限 loss/gradient/logit、超时、磁盘超限或 reload 失败都停止且不聚合。
5. 这是 reasoning-only diagnostic。只有当三 seed 完成、provenance 合格且未触发停止条件，才可申请另一个“七头正式候选”合同；本诊断永远不命名为 `M2_SELECTED_CANDIDATE`。
...
- 对推荐的 reasoning corrective 诊断，额外要求 reasoning Macro-F1 mean 至少比 S1 高 0.01，三 seed mean Micro-F1 和 exact-set accuracy 不得下降超过 0.01，`NO_REASON_GIVEN`（support 86）mean F1 不低于 S1 且 worst-seed drop 不超过 0.03；同时不得让已触发的其他关键边界超过 0.05 drop。
- 三 seed 的每个 head sample standard deviation ≤0.05；reasoning candidate 若任何 seed 仍有 `NO_REASON_GIVEN` 的严重回归、非有限输出或 device-stratified mismatch，则拒绝，不进入七头正式合同。
...
只运行 reasoning_tags 三 seed 35/71/107；Train 1,822 拟合，Dev 448 诊断。
冻结 embeddings 与前两个 Transformer blocks，只解冻最后一个 block 和 reasoning head。
...
禁止 Test/Anchor/Gold/OOD/reference predictions/LLM/云/生产推理；结果只能是 diagnostic。
若 reasoning 门槛失败，停止；LERT-small fallback 需另行授权，不自动下载或训练。
```
