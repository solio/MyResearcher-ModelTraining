...
def _config(contract: Mapping[str, Any], order: Mapping[str, Sequence[str]]) -> dict[str, Any]:
    ...
    return {"stage_id": STAGE_ID, "model_id": MODEL_ID, "revision": REVISION, "license": LICENSE, "local_files_only": True, "trust_remote_code": False, "initialization_source": "OFFICIAL_FIXED_RBT3_ORIGINAL_WEIGHTS_NO_WARM_START", "shared_components": ["tokenizer", "input_builder", "embeddings", "transformer_blocks_0_and_1"], "per_head_components": "independent_transformer_block_2_and_output_head", "max_length": MAX_LENGTH, "truncation": "HEAD_TAIL", "padding": common["padding"], "token_type_ids": "NOT_EMITTED", "batch_size": BATCH_SIZE, "head_dropout": common["head_dropout"], "stock_code_token_cap": common["stock_code_token_cap"], "stock_name_token_cap": common["stock_name_token_cap"], "class_order": {head: list(order[head]) for head in V1_HEADS}, "optimizer": {"name": "AdamW", "block_learning_rate": BLOCK_LR, "head_learning_rate": HEAD_LR, "weight_decay": WEIGHT_DECAY, "betas": optimizer["betas"], "epsilon": optimizer["epsilon"]}, "stopping": {"max_epochs": MAX_EPOCHS, "patience": PATIENCE, "minimum_delta": 0.0}, "gradient_clipping_max_norm": 1.0, "fit_population": "Train_1822_only", "dev_role": "early_stopping_and_diagnostic_only", "sealed_roles_not_loaded": ["Test", "Anchor", "Gold", "OOD", "reference_predictions"], "selected_candidate": False}
