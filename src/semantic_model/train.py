...
def _reference_comparison(
    baseline_report: Mapping[str, Any], anchor_metrics: Mapping[str, Any]
) -> dict[str, Any]:
    if not baseline_report.get("reproduction_contract_complete"):
        return {
            "claim_allowed": False,
...
def train_prepared(
    ...
    *,
    ...
    comparison = _reference_comparison(baseline_report, anchor_metrics)
    if comparison["claim_allowed"] and comparison["within_tolerance"]:
    ...
    try:
        ...
        run_manifest: dict[str, Any] = {
            **identity_payload,
            ...
            "split_manifest_id": read_json(config.data_path("split_manifest")).get(
                "split_manifest_id"
            ),
            "quarantine_manifest_id": read_json(
                config.data_path("quarantine_manifest")
            ).get("quarantine_manifest_id"),
            "anchor_manifest_id": read_json(config.data_path("anchor_manifest")).get(
                "anchor_manifest_id"
            ),
            "canonical_package_manifest_id": read_json(
                config.data_path("package_manifest")
        ...
        raise
    return {"status": status, "run_id": run_id, "run_dir": str(final_dir)}
